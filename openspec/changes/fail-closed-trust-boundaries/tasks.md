...
**Status:** Proposed; implementation has not yet satisfied acceptance.
...
- [ ] **1.1 (P0, M)** Add the invocation input descriptor and Git index snapshot allocator.
...
- [ ] **1.2 (P0, M)** Route hook configuration, scope, gates, and external steps through the snapshot root.
...
- [ ] **1.3 (P0, M)** Add opposing index/working-tree architecture, fmt, and lint fixtures.
...
- [ ] **2.1 (P0, M)** Extend leases with canonical project, resource kind,
...
- [ ] **2.2 (P0, M)** Add runtime inspect and pre-remove ownership verification.
  **Acceptance:** Cleanup invokes remove only after fresh inspection proves
  full label and immutable-ID equality.
- [ ] **2.3 (P0, M)** Add forged, cross-project, malformed, active, renamed, and
...
- [ ] **3.1 (P0, M)** Implement the root-confined atomic publisher.
...
- [ ] **3.2 (P0, M)** Migrate reports, parsed logs, cleanup evidence, migrations,
  and generated schemas to the publisher.
  **Acceptance:** No predictable repository-adjacent output bypasses the shared boundary.
- [ ] **3.3 (P0, S)** Add target/parent symlink, directory, collision, and
...
- [ ] **4.1 (P0, M)** Add the invocation artifact registry and declaration bindings.
  **Acceptance:** Required artifacts carry invocation, optional step, kind,
  normalized path, size, and digest.
- [ ] **4.2 (P0, M)** Implement report/registry/manifest/disk closed-set validation and manifest-last publication.
  **Acceptance:** All four views agree before `evidence_complete` can become true.
- [ ] **4.3 (P0, M)** Add missing, unlink-open, symlink, stale-invocation,
...
- [ ] **5.1 (P0, M)** Generate a deterministic machine-readable release inventory.
  **Acceptance:** Every platform binary and the CycloneDX SBOM has an explicit identity.
- [ ] **5.2 (P0, M)** Drive checksum, signing, attestation, verification, and upload from the inventory.
  **Acceptance:** Independent globs do not define integrity or upload subject sets.
- [ ] **5.3 (P0, M)** Add an offline release fixture for exact-set and tamper verification.
  **Acceptance:** Missing, extra, modified, unsigned, unattested, or unlisted assets block publication.

## 6. Closeout

- [ ] **6.1 (P0, S)** Run locked tests, formatter, strict Clippy, dependency audit,
...
- [ ] **6.2 (P0, S)** Synchronize ADR, README/configuration, schemas, changelog,
