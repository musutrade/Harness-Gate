...
- [ ] 3.1 Design and implement a deterministic migration/import path for structurally compatible `.arc-flow/flow.toml` execution configuration.
- [ ] 3.2 Preserve aliases, components, scope, services, parsers, profiles, required steps, dependencies, environment handling and timeouts where semantics match.
- [ ] 3.3 Fail visibly on unsupported or lossy mappings; never silently drop a required Arc-Admin blocker.
- [ ] 3.4 Measure manual migration effort and configuration duplication as product UX metrics.
