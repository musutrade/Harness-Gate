...
The wrapper MUST record each successful dep-info producer's output digest and byte
length. Capture and verification MUST match the complete raw output against that
identity, including environment comments, without claiming full environment closure.

#### Scenario: Changed raw dep-info with an unchanged file set

- GIVEN real compiler output whose captured environment comment is added, changed or omitted
- WHEN a test re-anchors the outer manifest but preserves the producer record
- THEN verification rejects the byte mismatch; missing or inconsistent producer identities also fail.
