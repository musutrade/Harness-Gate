...
use config::{
    parse_audit_config, validate_audit_config, AllowlistEntry, BlockCommentSyntax, CommentSyntax,
    Config, EngineConfig, HardRule, StringSyntax,
};
use report::{generate_markdown, generate_report};
use scanner::{resolve_rule_roots, scan_arch_rules, scan_files};

#[cfg(test)]
use config::{ArchRule, PathsConfig, AUDIT_CONFIG_VERSION};
#[cfg(test)]
use scanner::{compile_regexes, is_allowlisted};
