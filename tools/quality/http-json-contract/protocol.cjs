...
const COLLECTOR={name:'http-json-contract',version:'0.1.0-rc.3'};
...
function discover(request) {
 ...
 const subjects=[[request.component,p.boundary,p.contract],[p.consumer,p.consumer_boundary,p.client]].map(([component,boundary,name])=>{
...
function collect(request) {
 ...
 consumerInventory(sourceFiles,p.client);
 ...
 const values=measure(parse(base.toString()),parse(head.toString()),type.toString(),client.toString(),p.type_name,parse(observations.toString()));
 ...
 const parseConsumer=require('./measure.cjs').consumer;
 assert.equal(canonical(parseConsumer(type.toString(),client.toString(),p.type_name).fields),canonical(parseConsumer(expectedGenerated,client.toString(),p.type_name).fields),'generated type was edited without regeneration');
 values['contract.client_drift']={type:'boolean',value:origin!==sha(head)};
 ...
 for(const [name,bytes] of [['head.json',head],['baseline.json',base],['observations.json',observations],['receipt.json',Buffer.from(canonical(receipt))],['response.ts',type],['client.ts',client],['expected-generated.ts',generated]]) {
 ...
 metrics:Object.entries(values).map(([name,value])=>({name,value,artifacts:refs})),capabilities:Object.keys(TYPES).map(metric=>({metric,state:'supported',reason:'complete declared GET response variants and consumer AST',artifacts:refs})),artifacts,status:'measured',
