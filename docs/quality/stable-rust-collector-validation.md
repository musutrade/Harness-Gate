...
The candidate binary is **1,721,832 bytes**, SHA-256
`f4f71cc58f51194ab4b9c9c976bd34360ddb0bf471c414f7c7439949353ee81e`.
...
The actual unsigned candidate package occupies 2,775,335 bytes: program 1,721,832,
...
The local test-only signed package and one installed version occupy 2,775,804
bytes each. Two retained versions occupy 5,551,608 bytes. Interrupted staging
occupies 2,775,804 bytes; the exercised root totals 8,328,027 bytes including
...
under `target/gh-259/acceptance-10/` in the execution workspace; the summary is
...
`target/gh-259/core-acceptance-{plain,boundaries,features}-06/`. Each invokes the
...
| --- | --- |
| `CARGO_TARGET_DIR="$PWD/target/gh-259/stable-target" cargo +1.97.1 build --manifest-path tools/quality/rust-stable-collector/Cargo.toml --release --locked --offline` | Passed initial build; final artifact rebuilt by preparation below; `stable-package-build-01.log` |
| Same target, `cargo +1.97.1 test --manifest-path tools/quality/rust-stable-collector/Cargo.toml --locked --offline` | 8 passed; `stable-package-tests-01.log` |
| `cargo +1.97.1 fmt --manifest-path tools/quality/rust-stable-collector/Cargo.toml -- --check` | Passed; `stable-package-fmt-01.log` |
| Same target, `cargo +1.97.1 clippy --manifest-path tools/quality/rust-stable-collector/Cargo.toml --all-targets --locked -- -D warnings` | Passed; `stable-package-clippy-01.log` |
| `python3 tools/quality/rust-stable-collector/validate_stable_candidate.py --binary target/gh-259/stable-target/release/harness-gate-rust-stable-collector --output target/gh-259/acceptance-10` | 23 passed; `acceptance-10.log` |
...
| `CARGO_TARGET_DIR="$PWD/target/gh-259/quality-target" python3 -m unittest discover -s tools/quality/tests -v` | 444 tests OK, 36 skipped; `quality-tests-05.log` |
| `python3 -m unittest discover -s tools/release/tests -v` | 91 passed; `release-tests-02.log` |
...
| `CARGO_TARGET_DIR="$PWD/target/gh-259/core-target" python3 tools/quality/docs_consistency.py --output target/quality/docs-consistency.json` | Passed; `docs-consistency-06.log` |
...
| `target/gh-259/core-target/debug/examples/stable_collector_acceptance target/gh-259/stable-target/release/harness-gate-rust-stable-collector target/gh-259/core-target/debug/harness-gate target/gh-259/acceptance-10 target/gh-259/core-acceptance-${fixture}-06 $fixture` for `plain boundaries features` | All three passed; matching `core-acceptance-${fixture}-06.log` |
| `python3 tools/quality/rust-stable-collector/compare_historical_fixture.py --binary target/gh-259/stable-target/release/harness-gate-rust-stable-collector --output target/gh-259/historical-03` | Passed; `historical-03.log` |
| `python3 tools/quality/rust-stable-collector/validate_lifecycle.py --binary target/gh-259/stable-target/release/harness-gate-rust-stable-collector --package target/gh-259/package-02/unsigned-package --output target/gh-259/lifecycle-04` | 33 passed; real RSA, mocked Sigstore; `lifecycle-04.log` |
| `python3 tools/quality/rust-stable-collector/prepare_release.py --output target/gh-259/package-02 --target-dir "$PWD/target/gh-259/stable-target" --toolchain 1.97.1 --acceptance target/gh-259/acceptance-10/summary.json ab145fc1223b9c75e98dcd8177548e564912cb219a4a2acb863c5e726e0b741e` | Passed locked offline build and package preparation; `package-02.log` and `package-02/preparation.json` |
