...
- The stable first-party collector release contract uses SHA-256 payload checks and one Sigstore signature bound to the exact release version, with verified installation and failure recovery. Do not add an RSA release signature, a separate private-candidate signing round or duplicate collector-specific approval packets. This delivery simplification does not waive real measurement, required quality/CRAP gates, measurement-series review or the migration publication hold.
...
- For a diagnosed procedural-macro gap, prefer one ordinary Rust generation library shared by the macro entry and analysis, with optional structured observations from that same generation path. Bind source/version, invocation, configuration, target and generated owners. TokenStream output alone is not final expansion or coverage proof. Keep expansion size, cognitive complexity and CRAP cyclomatic complexity distinct; do not introduce unstable expansion/statistics interfaces or mandatory user visualization/tracing tools.
