...
| **R-08** | 实现已进入收口：[install.sh](../install.sh) 现在要求显式 immutable tag，下载到临时目录并校验 SHA256/Sigstore 后原子安装；离线 fixture 已覆盖篡改、签名失败和 symlink。 | 等待 [installer-integrity](../openspec/changes/installer-integrity/proposal.md) PR 和合并后 `main` CI 全绿，再将 ADR-0036/OpenSpec 标记为 Accepted/Implemented；下一次真实 release 需补发布元数据兼容性证据。 |
