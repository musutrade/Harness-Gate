...
| **R-10** | 发布治理已进入实现收口：tag/source/版本/完整 `main` CI 的 eligibility 校验已接入 release workflow；tag ruleset [21989651](https://github.com/musutrade/Harness-Gate/rules/21989651) 与受保护 `release` environment 已配置。 | 等待 [release-governance-hardening](../openspec/changes/release-governance-hardening/proposal.md) PR 和合并后 `main` CI 全绿，再将 ADR-0035/OpenSpec 标记为 Accepted/Implemented；CI 工具安装与 coverage 选择作为独立维护债，不扩大 R-10 发布阻断边界。 |
